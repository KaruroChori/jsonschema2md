 ...
 * @param {Function} [options.info] - (optional) Info logger
 * @param {Function} [options.debug] - (optional) Debug logger
 * @param {{ [key: string]: string }} [options.metaIndex] - (optional) Metadata to be added to Markdown
 * @returns {(markdownAst: MarkdownAst) => ReadmeContent}
 */
export function writereadme({
  out, error, info, metaIndex,
}) {
  const processor = unified()
    .use(gfm)
    .use(stringify);
  ...
  return (markdownAst) => {
    // add YAML frontmatter
    const content = (!metaIndex ? '' : '---\n')
      + (!metaIndex ? '' : yaml.dump(metaIndex))
      + (!metaIndex ? '' : 'title: Schemas\n')
      + (!metaIndex ? '' : '---\n\n')
