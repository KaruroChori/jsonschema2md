 ...
 * @param {Function} [options.info] - (optional) Info logger
 * @param {Function} [options.debug] - (optional) Debug logger
 * @param {{ [key: string]: string }} [options.metadataIndex] - (optional) Metadata to be added to Markdown
 * @returns {(markdownAst: MarkdownAst) => ReadmeContent}
 */
export function writereadme({
  out, error, info, metadataIndex,
}) {
  const processor = unified()
    .use(gfm)
    .use(stringify);
  ...
  return (markdownAst) => {
    // add YAML frontmatter
    const content = (!metadataIndex ? '' : '---\n')
      + (!metadataIndex ? '' : yaml.dump(metadataIndex))
      + (!metadataIndex ? '' : 'title: Index\n')
      + (!metadataIndex ? '' : '---\n\n')
