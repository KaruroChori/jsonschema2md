...
export function jsonschema2md(schema, options) {
  ...
  let metaIndex = options.metadataIndex;
...
export async function main(args) {
  ...
  const { argv } = yargs(args)
    ...
    .alias('mx', 'meta-index')
    .describe('mx', 'add metadata elements to .md index files Eg -mx template=reference. Multiple values can be added by repeating the flag Eg: -mx template=reference -mx hide-nav=true')
    .coerce('mx', (n) => n && pipe(
      // turn this into an object of key value pairs
      iter(n),
      map((i) => i.split('=')),
