...
export async function main(args) {
  ...
  const { argv } = yargs(args)
    ...
    .alias('mi', 'metaIndex')
    .describe('mi', 'add metadata elements to .md index files Eg -mi template=reference. Multiple values can be added by repeating the flag Eg: -mi template=reference -mi hide-nav=true')
    .coerce('mi', (mi) => mi && pipe(
      // turn this into an object of key value pairs
      iter(mi),
      map((i) => i.split('=')),
      obj,
